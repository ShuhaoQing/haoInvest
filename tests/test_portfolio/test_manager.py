...
class TestBuySell:
    def test_single_buy(self, db: Database):
        ...
        pm.add_trade(Transaction(
            ...
            fee=5.0, executed_at=datetime(2026, 3, 28),
        ))
    ...
    def test_multiple_buys_weighted_avg(self, db: Database):
        ...
        pm.add_trade(Transaction(
            symbol="600519", market_type=MarketType.A_SHARE,
            action=TransactionAction.BUY, quantity=100, price=1800.0,
    ...
    def test_partial_sell(self, db: Database):
        ...
        pm.add_trade(Transaction(
            symbol="600519", market_type=MarketType.A_SHARE,
            action=TransactionAction.BUY, quantity=200, price=1700.0,
            executed_at=datetime(2026, 3, 1),
        ))
        pm.add_trade(Transaction(
            symbol="600519", market_type=MarketType.A_SHARE,
            action=TransactionAction.SELL, quantity=100, price=1800.0,
            executed_at=datetime(2026, 3, 15),
        ))
        pos = pm.get_holding("600519", MarketType.A_SHARE)
        assert pos is not None
    ...
    def test_full_sell_zeroes_position(self, db: Database):
        ...
        pm.add_trade(Transaction(
            symbol="600519", market_type=MarketType.A_SHARE,
            action=TransactionAction.BUY, quantity=100, price=1700.0,
            executed_at=datetime(2026, 3, 1),
        ))
        pm.add_trade(Transaction(
            symbol="600519", market_type=MarketType.A_SHARE,
    ...
    def test_sell_with_fees_and_tax(self, db: Database):
        ...
        pm.add_trade(Transaction(
            symbol="600519", market_type=MarketType.A_SHARE,
            action=TransactionAction.BUY, quantity=100, price=1700.0,
            fee=5.0, tax=0, executed_at=datetime(2026, 3, 1),
        ))
        pm.add_trade(Transaction(
            symbol="600519", market_type=MarketType.A_SHARE,
            action=TransactionAction.SELL, quantity=50, price=1800.0,
            fee=5.0, tax=45.0, executed_at=datetime(2026, 3, 15),
        ))
        pos = pm.get_holding("600519", MarketType.A_SHARE)
        assert pos is not None
    ...
    def test_small_crypto_trades(self, db: Database):
        ...
        for _ in range(3):
            pm.add_trade(Transaction(
                ...
                action=TransactionAction.BUY, quantity=0.001, price=87000.0,
                executed_at=datetime(2026, 3, 28),
            ))
        pm.add_trade(Transaction(
            symbol="BTC_USDT", market_type=MarketType.CRYPTO,
            action=TransactionAction.SELL, quantity=0.003, price=88000.0,
            executed_at=datetime(2026, 3, 28),
        ))
...
class TestSplitAndDividend:
    def test_stock_split(self, db: Database):
        ...
        pm.add_trade(Transaction(
            symbol="600519", market_type=MarketType.A_SHARE,
            action=TransactionAction.SPLIT, quantity=0, price=2.0,
            executed_at=datetime(2026, 3, 1),
        ))
        pos = pm.get_holding("600519", MarketType.A_SHARE)
        assert pos is not None
...
class TestPortfolioSummary:
    def test_allocation_percentages(self, db: Database):
        ...
        pm.add_trade(Transaction(
            symbol="600519", market_type=MarketType.A_SHARE,
            action=TransactionAction.BUY, quantity=100, price=1000.0,
            executed_at=datetime(2026, 3, 1),
        ))
        pm.add_trade(Transaction(
            symbol="000858", market_type=MarketType.A_SHARE,
            action=TransactionAction.BUY, quantity=100, price=1000.0,
            executed_at=datetime(2026, 3, 1),
        ))
